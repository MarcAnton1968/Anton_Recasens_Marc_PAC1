...
Abans que res, carreguem les dades al nostre entorn de treball. Treballarem amb un dels exemples de mostra, veure **[aquí](assa)**. Aquest exemple porta dos arxius, DataInfo_S013 i DataValues_S013. Carreguem els dos fitxers:
...
Un pas important ara que hem vist com són les variables pot ser fer un Anàlisi de Components Principals (PCA)). Com que tenim els NAs eliminats i les dades normalitzades, podem fer-lo sense problemes addicionals. Reduim la PCA a 5 components ja que sinó en fa 39 i el resultat s'allarga molt
...
Tot i això, per veure com evoluciona l'explicació de la variança faig un gràfic de barres amb una nova PCA que porta tots els components 

```{r}
barplot(summary(PCAseMA)$importance[2,],ylim=c(0,0.6))
title(main="Proportion of variation",line=1)
```
S'parecia força clar com la primera aporta força i les següents van afegint poc. Tot i això, podriem dir que fins a la cinquena sembla interessant agafarles.

Grafiquem ara les primeres components i ho fem en relació al tipus de cirugia que és el que cerca l'article original. De moment, dues

```{r}
# carreguem les dades pel gràfic
cirugia <- as.factor(rowData(seMA)$SURGERY)
cirugian <- as.numeric(cirugia)
# Amb això hem passat les dades de cirugia amb 1: by pass i 2: tubular
...
Sembla que les dues components ens separen els dos tipus de cirugia, el que implicaria una resposta diferent dels metabolits a la cirugia. Provem si amb 3 dimensions millora
...
Sembla que en 3 dimensions encara es veu més l'efecte de la cirugia

Amb plotly ho podem veure més facil al rotar el gràfic. El possem per fer el html, però no per fer el pdf, ja que no funciona amb el latex. 

```{r}
#library(plotly)
#fig <- plot_ly(gr3Ddata, 
#               x = ~gr3Ddata[,1], y = ~gr3Ddata[,2], z = ~gr3Ddata[,3],
#               color = ~cirugia, 
#               type = 'scatter3d', 
#               mode = 'markers', 
#               marker = list(size = 5))
